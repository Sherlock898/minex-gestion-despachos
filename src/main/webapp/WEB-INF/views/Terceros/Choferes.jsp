		...
		<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
			...
			<div class="btn-toolbar mb-2 mb-md-0">
				...
				<c:if test="${usuario.rol == 'ADMIN'}">
					<button type="button" class="btn btn-sm btn-outline-secondary" onclick="location.href = '/choferes/add'">Agregar</button>
				</c:if>
			</div>	
		</div>
	<div class="table-responsive small">
		<table id="choferesData" class="table table-striped table-sm text-center">
			<thead>
				<tr>
					<th>Id</th>
					<th>Rut</th>
			...
			<tbody>
				<c:forEach var="chofer" items="${choferes}">
					<tr>
						...
						<td>${chofer.rut}</td>
						<td>${chofer.nombre}</td>
						<td>${chofer.activo ? "Si" : "No"}</td>
						<c:if test="${usuario.rol == 'ADMIN'}">
							<td>
								<button type="button" class="btn btn-sm btn-outline-primary" onclick="location.href = '/choferes/${chofer.id}/edit'">Editar</button>
...
<script>
	...
	initializeDataTable('choferesData', 'copyClipboard', 'exportExcel', 4);
